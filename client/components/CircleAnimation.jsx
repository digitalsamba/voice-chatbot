...
const CircleAnimation = ({ audioContext, analyser, isMuted, aiState }) => {
  ...
  useEffect(() => {
    ...
    const initCircles = () => {
      circles.current = [];
      for(let idx = 0; idx < 3; idx++) { // Уменьшено до 3 кругов
        const swingpoints = [];
    ...
    const animate = () => {
      ...
      const targetMultiplier = aiStateRef.current === 'thinking' ? 0.95 :
        aiStateRef.current === 'speaking' ? 1.15 : 1;
      ...
      circles.current.forEach((swingpoints, k) => {
        ...
        // Убрано обращение к colorStops
        ctx.shadowColor = '#70d7ff55'; // Фиксированный цвет тени
